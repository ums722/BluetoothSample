...
import com.ums722.bluetoothsample.domain.ConnectionResult
...
class AndroidBluetoothController(private val context: Context) : BluetoothController {
    ...
    private val bluetoothStateReceiver = BluetoothStateReceiver {isConnected, bluetoothDevice ->
        if (bluetoothAdapter?.bondedDevices?.contains(bluetoothDevice) == true){
            ...
            CoroutineScope(Dispatchers.IO).launch{
                _errors.emit("cant connect to a non paired devicec.")
            }
        }
    }
    ...
    override fun connectToDevice(device: BluetoothDeviceEntity): Flow<ConnectionResult> {
        return flow {
            if (!hasPermission(Manifest.permission.BLUETOOTH_CONNECT)) {
                throw SecurityException("No BlueTooth_Connect permission")
            }
           ...
           val bluetoothDevice = bluetoothAdapter?.getRemoteDevice(device.address)
            ...
            currentClientSocket?.let { socket ->
                ...
                try {
                    ...
                    emit(ConnectionResult.Error("Connection wa interrupted"))
                }
            }
        }.onCompletion {
            closeConnection()
    ...
    companion object {
        const val SERVICE_UUID = "27b7dida-08c7-4505-a6d1-2459987e5e2d"
    }
}